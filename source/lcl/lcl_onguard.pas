...
{$IFDEF FPC}
 uses
 {$IFNDEF NoMakeCodesSupport} lcl_onguard2 {$ENDIF}
 {$IFNDEF NoMakeKeysSupport} , lcl_onguard3{$ENDIF}
 ;
{$ENDIF}
...
begin
  ...
  DoOnGetKey(lKey);
  lCode := DoOnGetCode;
  lModifier := DoOnGetModifier;

  ApplyModifierToKeyPrim(lModifier, lKey, SizeOf(lKey));
  if IsDateCodeValid(lKey, lCode) then begin
    if IsDateCodeExpired(lKey, lCode) then
...
begin
  ...
  ApplyModifierToKeyPrim(lModifier, lKey, SizeOf(lKey));
  Result := GetDateCodeValue(lKey, lCode);
end;
...
begin
  ...
  ApplyModifierToKeyPrim(lModifier, lKey, SizeOf(lKey));
  if IsDaysCodeValid(lKey, lCode) then begin
    if IsDaysCodeExpired(lKey, lCode) then begin
      Result := ogDayCountUsed;
      if GetExpirationDate(lKey, lCode) < Date then
...
begin
  ...
  DecDaysCode(lKey, lWork);

  {save code if it was changed}
  if (lWork.CheckValue <> lCode.CheckValue) or (lWork.Days <> lCode.Days) then
    DoOnChangeCode(lWork);
end;
...
begin
  ...
  ApplyModifierToKeyPrim(lModifier, lKey, SizeOf(lKey));
  Result := GetDaysCodeValue(lKey, lCode);
end;
...
var
  F : TLCLCodeGenerateFrm;
{$ENDIF}                                                             {!!.10}
begin
{$IFNDEF NoMakeCodesSupport}                                         {!!.10}
  F := TLCLCodeGenerateFrm.Create(Owner);
  try
...
begin
{$IFNDEF NoMakeCodesSupport}                                         {!!.10}
  F := TLCLKeyMaintFrm.Create(Owner);
  try
    F.SetKey(FKey);                                                  {!!.08}
    F.KeyType := FKeyType;
    F.KeyFileName := FKeyFileName;
