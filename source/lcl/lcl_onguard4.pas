...
type
  TLCLEditProductFrm = class(TForm)
    OKBtn: TButton;
    CancelBtn: TButton;
    Panel1: TPanel;
    Label1: TLabel;
...
procedure TLCLEditProductFrm.FormCreate(Sender: TObject);
begin
  InfoChanged(Sender);
end;

procedure TLCLEditProductFrm.InfoChanged(Sender: TObject);
var
  Work : TKey;
begin
...
procedure TLCLEditProductFrm.KeyPasteSbClick(Sender: TObject);
begin
  KeyEd.PasteFromClipboard;
end;

procedure TLCLEditProductFrm.GenerateKeySbClick(Sender: TObject);
var
  F    : TLCLKeyGenerateFrm;
begin
  F := TLCLKeyGenerateFrm.Create(Self);
  try
    F.SetKey(FKey);
    F.KeyType := FKeyType;
...
function TLCLEditProductFrm.GetShowHints : Boolean;
begin
  Result := False;
  Result := ShowHint;
end;

procedure TLCLEditProductFrm.SetShowHints(Value : Boolean);
begin
  ShowHint := Value;
end;

procedure TLCLEditProductFrm.GetKey(var Value : TKey);
begin
  Value := FKey;
end;

procedure TLCLEditProductFrm.SetKey(Value : TKey);
begin
  FKey := Value;
end;
