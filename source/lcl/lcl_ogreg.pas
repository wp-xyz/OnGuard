...
uses
  Classes, Forms, lcl_ogabout0,
  ogconst, lcl_ognetwrk, lcl_ogproexe, ogfirst,
  lcl_onguard,
  lcl_onguard2,
  lcl_onguard3,
  lcl_onguard5,
...
begin
  if Index = 0 then begin
    ...
    with TLCLKeyMaintFrm.Create(Application) do
      try
        ShowHint := True;
...
{    TypeInfo(string), TOgCodeBase, 'Expires', TOgExpiresProperty);}  {!!.09}
  ...
  RegisterPropertyEditor(
    TypeInfo(string), TOgCodeBase, 'About', TOgLCLAboutProperty);       {!!.08}
  RegisterPropertyEditor(
    TypeInfo(string), TOgProtectExe, 'About', TOgLCLAboutProperty);     {!!.08}
  RegisterPropertyEditor(
    TypeInfo(string), TOgMakeCodes, 'About', TOgLCLAboutProperty);      {!!.08}
  RegisterPropertyEditor(
    TypeInfo(string), TOgMakeKeys, 'About', TOgLCLAboutProperty);       {!!.08}
