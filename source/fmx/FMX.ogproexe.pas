...
{$IFDEF DELPHI16UP}[ComponentPlatformsAttribute(pidWin32 or pidWin64)]{$ENDIF}
  TOgFMXProtectExe = class(TComponent)
  protected {private}
    {property variables}
    FAutoCheck  : Boolean;
    FCheckSize  : Boolean;
...
constructor TOgFMXProtectExe.Create(AOwner : TComponent);
begin
...
function TOgFMXProtectExe.CheckExe(Report : Boolean) : TExeStatus;
begin
...
procedure TOgFMXProtectExe.DoOnChecked(Status : TExeStatus);
begin
  if Assigned(FOnChecked) then FOnChecked(Self, Status);
end;

function TOgFMXProtectExe.GetAbout : string;                            {!!.08}
begin
  Result := OgVersionStr;
end;

procedure TOgFMXProtectExe.Loaded;
begin
  inherited Loaded;
...
procedure TOgFMXProtectExe.SetAbout(const AValue : string);              {!!.08}
begin
  // do nothing
end;

function TOgFMXProtectExe.StampExe(const FileName : string ;  EraseMarker : Boolean) : Boolean;
begin
  Result := ProtectExe(FileName,  EraseMarker);
end;

function TOgFMXProtectExe.UnStampExe(const FileName : string) : Boolean;
begin
