...
{$IFDEF DELPHI16UP}[ComponentPlatformsAttribute(pidWin32 or pidWin64)]{$ENDIF}
  TOgFMXNetCode = class(TOgFMXCodeBase)
  protected {private}
    {property variables}
    FFileName        : string;
...
function TOgFMXNetCode.CheckCode(Report : Boolean) : TCodeStatus;
var
  Key      : TKey;
begin
  Result := ogValidCode;
...
{!!.01}
constructor TOgFMXNetCode.Create(AOwner : TComponent);
begin
...
function TOgFMXNetCode.CreateAccessFile : Boolean;
var
  ACode     : TCode;
  Key      : TKey;
  AModifier : ogLongInt;
...
destructor TOgFMXNetCode.Destroy;
begin
...
{!!.02}
function TOgFMXNetCode.DoOnGetFileName : string;
begin
  Result := '';
...
function TOgFMXNetCode.GetActiveUsers : ogLongInt;
var
  Key           : TKey;
  AModifier      : ogLongInt;
...
function TOgFMXNetCode.GetInvalidUsers : ogLongInt;
var
  Key           : TKey;
  AModifier      : ogLongInt;
...
function TOgFMXNetCode.GetMaxUsers : ogLongInt;
var
  Key      : TKey;
  AModifier : ogLongInt;
begin
...
function TOgFMXNetCode.IsRemoteDrive(const ExePath : string) : Boolean;
begin
  Result := IsAppOnNetwork(ExePath);
end;

procedure TOgFMXNetCode.Loaded;
var
  Key      : TKey;
  ACode     : TCode;
  AModifier : ogLongInt;
...
function TOgFMXNetCode.ResetAccessFile : Boolean;
var
  Key      : TKey;
  AModifier : ogLongInt;
begin
